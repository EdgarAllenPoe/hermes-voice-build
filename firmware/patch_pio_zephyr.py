...
with AttributeError: SCons.Node.FS.File has no attribute insert. Keep the same
library node and ordering, but supply the required one-element list.
No library, hardware setting, warning, or validation is removed.

Normal build PRE scripts run after required package installation. Clean-only
runs deliberately skip framework installation, so they must skip this patch.
"""
...
def patch_adapter(environment):
    platform = environment.PioPlatform()
...
if env.IsCleanTarget():
    print('Hermes adapter fix: clean-only invocation; no framework is needed')
else:
    patch_adapter(env)
