...
static int torn_bytes=-1,erase_failure;
...
int flash_write(const struct device*d,off_t o,const void*p,size_t n){
 ...
 if(torn_bytes>=0){size_t limit=(size_t)torn_bytes<n?(size_t)torn_bytes:n;torn_bytes=-1;
   for(size_t i=0;i<limit;i++)memory[o+i]&=src[i];return -EIO;}
 for(size_t i=0;i<n;i++)memory[o+i]&=src[i];return 0;
}
int flash_erase(const struct device*d,off_t o,size_t n){(void)d;if(o<0||(size_t)o+n>sizeof(memory)||o%4096||n%4096)return -1;if(erase_failure)return -EIO;memset(memory+o,255,n);return 0;}
...
void test_reboot(void){memset(slots,0,sizeof(slots));sequence=0;allocate_next=0;atomic_store(&hvb_recording,0);failure=-1;torn_bytes=-1;erase_failure=0;flash_errors=0;}
...
void test_torn_write(int n){torn_bytes=n;}
void test_erase_error(int yes){erase_failure=yes;}
void test_flip(int s,int at){memory[base(s)+at]^=1;}
