...
public final class MainActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state){
        ...
        TextView brand=label(BuildConfig.CI_BUILD?"HERMES VOICE Ã‚Â· PREVIEW":"HERMES VOICE",12,colors.accent,true);
    ...
    private void buildStatus(LinearLayout page){
        ...
        relayLabel=label("Checking relayÃ¢â‚¬Â¦",12,colors.muted,false);relayRow.addView(relayLabel);page.addView(relayRow);
        ...
        heroSymbol=label("Ã¢â‚¬Â¢",24,colors.amber,true);heroSymbol.setGravity(Gravity.CENTER);
        ...
        heroDetail=label("Reading your phone and recorder statusÃ¢â‚¬Â¦",15,colors.muted,false);add(hero,heroDetail,8);
    ...
    private void buildDiagnostics(LinearLayout page){
        ...
        diagnosticText=label("Reading diagnosticsÃ¢â‚¬Â¦",13,colors.text,false);diagnosticText.setTypeface(Typeface.MONOSPACE);
        ...
        add(page,label("Hermes Voice "+BuildConfig.VERSION_NAME+" Ã‚Â· "+(BuildConfig.DEBUG?"test build":"release build"),12,colors.muted,false),20);
    }
    ...
    private void render(DashboardReader.Snapshot snapshot){
        ...
        checkButton.setAlpha(s.checking?.65f:1f);setText(checkButton,s.checking?"Checking connectionÃ¢â‚¬Â¦":"Check connection");
        setText(diagnosticCheck,s.checking?"Checking connectionÃ¢â‚¬Â¦":"Check connection");
    ...
    private void serverSettings(){
        ...
        token.setHint(Settings.prefs(this).contains("token")?"Saved Ã‚Â· leave blank to keep":"Paste your server token");content.addView(token);
    ...
    private void permissionSettings(){
        if(!permissions()||checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED){
            request();
            toast("If Android no longer shows a prompt, enable permissions in the phoneÃ¢â‚¬â„¢s app settings.");
    ...
    private static String symbol(DashboardState.Level l){return l==DashboardState.Level.GOOD?"Ã¢Å“â€œ":l==DashboardState.Level.ERROR?"!":"Ã¢â‚¬Â¢";}
    private final class HealthCard {
        ...
        HealthCard(String category){
            ...
            badge=label("Ã¢â‚¬Â¢ Checking",11,colors.amber,true);badge.setPadding(dp(9),dp(5),dp(9),dp(5));row.addView(badge);box.addView(row);
            title=label("CheckingÃ¢â‚¬Â¦",19,colors.text,true);add(box,title,10);
